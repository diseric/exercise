...
do { 
    ...
    '2' {
        ...
        ForEach ($client in Get-Content $clients) {
            ...
            Copy-Item -Path $dir\sysmon.exe -Destination \\$client\c$\Tools\Sysmon.exe;
            Copy-Item -Path $dir\ionstorm-config.xml -Destination \\$client\c$\Tools\ionstorm-config.xml;
            Copy-Item -Path $dir\$wazuh -Destination \\$client\c$\Tools\$wazuh;
            Copy-Item -Path $dir\install-sysmon.bat -Destination \\$client\c$\Tools\install-sysmon.bat;
			Copy-Item -Path $dir\install-wazuh.bat -Destination \\$client\c$\Tools\install-wazuh.bat;
            Copy-Item -Path $dir\install-velociraptor.bat -Destination \\$client\c$\Tools\install-velociraptor.bat;
            Copy-Item -Path $dir\uninstall-sysmon64.bat -Destination \\$client\c$\Tools\uninstall-sysmon64.bat;
            Copy-Item -Path $dir\uninstall-wazuh.bat -Destination \\$client\c$\Tools\uninstall-wazuh.bat
            Copy-Item -Path $dir\velociraptor-v0.6.0-windows-amd64.exe -Destination \\$client\c$\Tools\velociraptor.exe;
            Copy-Item -Path $dir\client.config.yaml -Destination \\$client\c$\Tools\client.config.yaml;
    ...
    '3' {
        ...
        Start-Process -filepath $psexec "@$dir\ips.txt -u $domain\Administrator -p $pass c:\Tools\install-sysmon.bat -accepteula" -wait;
    }
	'4' {
        ##psexec install wazzuh
        Write-Host "Installing Sysmon and Wazuh Agents"
        Start-Process -filepath $psexec "@$dir\ips.txt -u $domain\Administrator -p $pass c:\Tools\install-wazuh.bat -accepteula" -wait;
    }
    '5' {
        ##psexec delete wazzuh - uncomment the line below and run the line individually
        Write-Host "Uninstalling Sysmon"
        Start-Process -FilePath $psexec "@$dir\ips.txt -u $domain\Administrator -p $pass c:\Tools\uninstall-sysmon64.bat -accepteula";
        sleep 20
    }
    '6' {
        ##psexec delete wazzuh - uncomment the line below and run the line individually
        Write-Host "Uninstalling Wazuh"
        Start-Process -FilePath $psexec "@$dir\ips.txt -u $domain\Administrator -p $pass c:\Tools\uninstall-wazuh.bat -accepteula";
        sleep 20
    }
    '7' {
    ...
    '8' {
        ...
        Start-Process -filepath $psexec "@$dir\ips.txt -u $domain\Administrator -p $pass c:\Tools\install-velociraptor.bat";
    }
    '9' {
        ##psexec to run gpupdate /force
        Write-Host "running GP Update"
        Start-Process -filepath $psexec "@$dir\ips.txt -u $domain\Administrator -p $pass gpupdate /force";
    }
    '10' {
